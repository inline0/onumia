...
final class SshClient implements UploadPackTransport, ReceivePackTransport
{
    ...
    {
        return str_starts_with($url, 'ssh://') || preg_match('/^[^@]+@[^:]+:.+$/', $url) === 1;
    }
    /**
    ...
    public static function parseUrl(string $url): array
    {
        ...
        if (str_starts_with($url, 'ssh://')) {
            ...
            return ['user' => $parsed['user'] ?? 'git', 'host' => $parsed['host'] ?? '', 'port' => $parsed['port'] ?? 22, 'path' => $parsed['path'] ?? ''];
        }
        // SCP-style: user@host:path
        if (preg_match('/^([^@]+)@([^:]+):(.+)$/', $url, $matches)) {
            return ['user' => $matches[1], 'host' => $matches[2], 'port' => 22, 'path' => $matches[3]];
        }
    ...
    public function uploadPack(string $url, string $request): string
    {
        ...
        return $this->execute($parsed['host'], $parsed['port'], $parsed['user'], "git-upload-pack '{$parsed['path']}'", $request);
    }
    /**
    ...
    {
        $parsed = self::parseUrl($url);
        return $this->execute($parsed['host'], $parsed['port'], $parsed['user'], "git-receive-pack '{$parsed['path']}'", $request);
    }
    /**
    ...
    public function discoverRefs(string $url): RefDiscovery
    {
        ...
        $response = $this->execute($parsed['host'], $parsed['port'], $parsed['user'], "git-upload-pack '{$parsed['path']}'", '', \true);
        return RefDiscovery::parse(PktLine::decode($response));
    }
    public function discoverReceivePackRefs(string $url): RefDiscovery
    {
        $parsed = self::parseUrl($url);
        $response = $this->execute($parsed['host'], $parsed['port'], $parsed['user'], "git-receive-pack '{$parsed['path']}'", '', \true);
        return RefDiscovery::parse(PktLine::decode($response));
    }
    /**
    ...
    private function executeSsh2(string $host, int $port, string $user, string $command, string $input, bool $allowExitWithOutput): string
    {
        ...
        if (is_file($keyFile) && is_file($pubKeyFile)) {
            if (!@ssh2_auth_pubkey_file($connection, $user, $pubKeyFile, $keyFile)) {
                ...
                } else if (!@ssh2_auth_agent($connection, $user)) {
                    throw new ProtocolException("SSH authentication failed for {$user}@{$host}");
                }
            }
        } else if (!@ssh2_auth_agent($connection, $user)) {
        ...
        stream_set_blocking($stream, \true);
        if ($input !== '') {
            fwrite($stream, $input);
        }
    ...
    private function executeProcess(string $host, int $port, string $user, string $command, string $input, bool $allowExitWithOutput): string
    {
        ...
        $processCommand = $this->canUseDirectProcessCommand($ssh) ? $this->buildDirectProcessCommand($ssh, $host, $port, $user, $command, $identityFile, $knownHosts, $strict) : $this->buildShellProcessCommand($ssh, $host, $port, $user, $command, $identityFile, $knownHosts, $strict);
        $process = proc_open($processCommand, [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']], $pipes);
     ...
     * @return list<string>
     */
    private function buildDirectProcessCommand(string $ssh, string $host, int $port, string $user, string $command, string|false $identityFile, string|false $knownHosts, string|false $strict): array
    {
        $parts = [$ssh, '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=' . $this->timeout, '-p', (string) $port];
    ...
    private function buildShellProcessCommand(string $ssh, string $host, int $port, string $user, string $command, string|false $identityFile, string|false $knownHosts, string|false $strict): string
    {
        $parts = [escapeshellarg($ssh), '-o BatchMode=yes', '-o ' . escapeshellarg('ConnectTimeout=' . $this->timeout), '-p ' . (int) $port];
        if (is_string($identityFile) && $identityFile !== '') {
