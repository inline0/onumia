...
{
    public function __construct(private readonly string $lfsUrl, private readonly int $timeout = 60)
    {
    }
    /**
    ...
    {
        $body = json_encode(['operation' => $operation, 'transfers' => ['basic'], 'objects' => $objects], \JSON_UNESCAPED_SLASHES);
        $url = rtrim($this->lfsUrl, '/') . '/objects/batch';
        $context = stream_context_create(['http' => ['method' => 'POST', 'timeout' => $this->timeout, 'header' => implode("\r\n", ['Content-Type: application/vnd.git-lfs+json', 'Accept: application/vnd.git-lfs+json', 'User-Agent: Pitmaster-LFS/1.0']), 'content' => $body]]);
        ...
        foreach ($data['objects'] as $obj) {
            ...
            $results[] = ['oid' => $obj['oid'] ?? '', 'size' => $obj['size'] ?? 0, 'href' => $href, 'error' => $error];
        }
        return $results;
    }
    private function httpGet(string $url): string
    {
        $context = stream_context_create(['http' => ['method' => 'GET', 'timeout' => $this->timeout, 'header' => "User-Agent: Pitmaster-LFS/1.0\r\n"]]);
        $response = @file_get_contents($url, \false, $context);
        if ($response === \false) {
    ...
    {
        $context = stream_context_create(['http' => ['method' => 'PUT', 'timeout' => $this->timeout, 'header' => implode("\r\n", ['Content-Type: application/octet-stream', 'User-Agent: Pitmaster-LFS/1.0']), 'content' => $content]]);
