...
final class ConflictMarker
{
     ...
     */
    public static function mark(string $ours, string $theirs, string $oursLabel = 'HEAD', string $theirsLabel = 'incoming', ?string $base = null, string $baseLabel = 'base'): string
    {
        $result = "<<<<<<< {$oursLabel}\n" . self::ensureTrailingNewline($ours);
        if ($base !== null) {
            $result .= "||||||| {$baseLabel}\n" . self::ensureTrailingNewline($base);
        }
        $result .= "=======\n" . self::ensureTrailingNewline($theirs) . ">>>>>>> {$theirsLabel}\n";
        return $result;
    }
