...
final class ThreeWayMerge
{
     ...
     */
    public static function merge(string $base, string $ours, string $theirs, string $oursLabel = 'HEAD', string $theirsLabel = 'incoming', string $conflictStyle = 'merge', string $baseLabel = 'base'): array
    {
        // Trivial cases
        if ($ours === $theirs) {
        ...
        if ($base === $ours) {
            return ['content' => $theirs, 'clean' => \true, 'conflicts' => 0];
        }
        if ($base === $theirs) {
            return ['content' => $ours, 'clean' => \true, 'conflicts' => 0];
        }
        ...
        [$result, $conflicts] = self::mergeRegions($baseLines, $oursRegions, $theirsRegions, $oursLabel, $theirsLabel, $conflictStyle, $baseLabel);
        $content = implode("\n", $result);
        if ($content !== '' && (str_ends_with($base, "\n") || str_ends_with($ours, "\n") || str_ends_with($theirs, "\n")) && !str_ends_with($content, "\n")) {
            $content .= "\n";
        }
    ...
    private static function buildChangeRegions(array $baseLines, array $targetLines): array
    {
        ...
        while ($opIndex < $count) {
            ...
            $regions[] = ['start' => $start, 'end' => $baseIndex, 'lines' => $replacement];
        }
        return $regions;
    }
    /**
     ...
     *
     */
    private static function mergeRegions(array $baseLines, array $oursRegions, array $theirsRegions, string $oursLabel, string $theirsLabel, string $conflictStyle, string $baseLabel): array
    {
        $result = [];
        $conflicts = 0;
        $baseIndex = 0;
        ...
        while ($oursIndex < $oursCount || $theirsIndex < $theirsCount) {
            ...
            [$oursWindow, $theirsWindow, $windowEnd, $oursIndex, $theirsIndex] = self::collectWindow($oursRegions, $theirsRegions, $oursIndex, $theirsIndex, $nextStart);
            if ($oursWindow === []) {
            ...
            $conflicts++;
            $result[] = rtrim(ConflictMarker::mark(implode("\n", $oursChunk), implode("\n", $theirsChunk), $oursLabel, $theirsLabel, $conflictStyle === 'diff3' ? implode("\n", $baseChunk) : null, $baseLabel), "\n");
            $baseIndex = $windowEnd;
        }
     ...
     * }
     */
    private static function collectWindow(array $oursRegions, array $theirsRegions, int $oursIndex, int $theirsIndex, int $windowStart): array
    {
        $oursWindow = [];
        $theirsWindow = [];
        ...
        while ($progress) {
            $progress = \false;
            if (isset($oursRegions[$oursIndex]) && self::regionTouchesWindow($oursRegions[$oursIndex], $windowStart, $windowEnd)) {
                $oursWindow[] = $oursRegions[$oursIndex];
                $windowEnd = max($windowEnd, $oursRegions[$oursIndex]['end']);
                $oursIndex++;
                $progress = \true;
            }
            if (isset($theirsRegions[$theirsIndex]) && self::regionTouchesWindow($theirsRegions[$theirsIndex], $windowStart, $windowEnd)) {
