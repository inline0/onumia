...
final class TreeDiff
{
    ...
    public function __construct(private readonly ObjectDatabase $objects, private readonly string $algorithm = 'myers')
    {
    }
    /**
    ...
    private function detectRenames(array $results): array
    {
        ...
        foreach ($deleted as $di => $del) {
            ...
            if ($bestIdx !== null) {
                ...
                $other[] = new DiffResult($del->oldPath, $add->newPath, $hunks, \false, $del->oldHash, $add->newHash);
                $matched[$bestIdx] = \true;
                unset($deleted[$di]);
            }
        }
    ...
    private function readTreeEntries(?ObjectId $treeId): array
    {
        ...
        foreach ($tree->entries as $entry) {
            $entries[$entry->name] = ['hash' => $entry->hash->hex, 'mode' => $entry->mode, 'isTree' => $entry->isTree()];
        }
    ...
    private function diffInto(?ObjectId $oldTree, ?ObjectId $newTree, string $prefix, array &$results): void
    {
        ...
        foreach ($allNames as $name) {
            ...
            if (!$oldEntry['isTree'] && !$newEntry['isTree']) {
                $results[] = $this->makeDiffResult($path, $this->readBlobContent($oldEntry['hash']), $this->readBlobContent($newEntry['hash']), $oldEntry['hash'], $newEntry['hash']);
                continue;
            }
            if ($oldEntry['isTree']) {
                $this->diffInto(ObjectId::fromHex($oldEntry['hash']), null, $path, $results);
    ...
    private function makeDiffResult(string $path, string $oldContent, string $newContent, ?string $oldHash, ?string $newHash): DiffResult
    {
        if (MyersDiff::isBinary($oldContent) || MyersDiff::isBinary($newContent)) {
            return new DiffResult($path, $path, [], \true, $oldHash, $newHash);
        }
        $hunks = DiffAlgorithm::diff($oldContent, $newContent, $this->algorithm);
        return new DiffResult($path, $path, $hunks, \false, $oldHash, $newHash, $this->contentLacksTrailingNewline($oldContent), $this->contentLacksTrailingNewline($newContent));
    }
