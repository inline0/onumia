...
{
    public function __construct(private readonly string $gitDir, private readonly string $workDir)
    {
    }
    /**
    ...
    private function mainWorktree(?RefDatabase $refs = null): Worktree
    {
        ...
        return new Worktree(name: null, path: $this->workDir, gitDir: $this->gitDir, branch: $branch, head: $headId, isMain: \true, isDetached: $isDetached, isLocked: \false, lockReason: null);
    }
    private function readLinkedWorktree(string $name, string $wtDir, ?RefDatabase $refs = null): Worktree
    {
        ...
        return new Worktree(name: $name, path: $path, gitDir: $wtDir, branch: $branch, head: $headId, isMain: \false, isDetached: $isDetached, isLocked: $isLocked, lockReason: $lockReason ?: null);
    }
    ...
    private function validateWorktreeName(string $name): string
    {
        ...
        if (str_contains($name, '/') || str_contains($name, '\\') || str_contains($name, "\x00")) {
