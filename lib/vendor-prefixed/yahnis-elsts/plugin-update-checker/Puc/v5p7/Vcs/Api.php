...
namespace Onumia\Lib\YahnisElsts\PluginUpdateChecker\v5p7\Vcs;

use Onumia\Lib\Parsedown;
use Onumia\Lib\PucReadmeParser;
if (!class_exists(Api::class, \false)) {
    abstract class Api
    {
        ...
        protected function enableDownloadRequestFilter()
        {
            ...
            add_filter('upgrader_pre_download', [$this, 'addDownloadRequestFilters']);
            //WP 3.7+
        }
        /**
         * @var bool
        ...
        public function filterUpdateDownloadRequestArgs($requestArgs, $url = '')
        {
            ...
            if ($this->isAuthenticationEnabled() && !empty($authHeader) && !empty($this->downloadUrlPrefixRequiringAuth) && strpos($url, $this->downloadUrlPrefixRequiringAuth) === 0) {
        ...
        {
            if (empty($this->downloadUrlPrefixRequiringAuth) || strpos($location, $this->downloadUrlPrefixRequiringAuth) === 0) {
                return;
            }
            //Remove the header.
            $authorizationHeader = $this->getAuthorizationHeader();
