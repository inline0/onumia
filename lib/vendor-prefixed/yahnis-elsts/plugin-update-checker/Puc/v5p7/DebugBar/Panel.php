...
namespace Onumia\Lib\YahnisElsts\PluginUpdateChecker\v5p7\DebugBar;

use Onumia\Lib\YahnisElsts\PluginUpdateChecker\v5p7\UpdateChecker;
if (!class_exists(Panel::class, \false) && class_exists('Onumia\Lib\Debug_Bar_Panel', \false)) {
    class Panel extends \Onumia\Lib\Debug_Bar_Panel
    {
        /** @var UpdateChecker */
        ...
        private function displayStatus()
        {
            ...
            if (function_exists('Onumia\Lib\get_submit_button')) {
            ...
            if ($update !== null) {
                ...
                foreach ($fields as $field) {
                    if (property_exists($update, $field)) {
                        $this->row(ucwords(str_replace('_', ' ', $field)), isset($update->{$field}) ? esc_html($update->{$field}) : null);
                    }
                }
                echo '</table>';
            } else {
        ...
        public function row($name, $value)
        {
            if (is_object($value) || is_array($value)) {
                ...
                $value = '<pre>' . esc_html(print_r($value, \true)) . '</pre>';
            } else if ($value === null) {
