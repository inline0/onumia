...
final class GitHubReleaseUpdater {
	...
	private function prime_factory_versions( string $factory ): void {
		...
		$classes = array(
			'Plugin' . '\\UpdateChecker'    => 'Onumia\\Lib\\YahnisElsts\\PluginUpdateChecker\\v5p7\\Plugin\\UpdateChecker',
			'Theme' . '\\UpdateChecker'     => 'Onumia\\Lib\\YahnisElsts\\PluginUpdateChecker\\v5p7\\Theme\\UpdateChecker',
			'Vcs' . '\\PluginUpdateChecker' => 'Onumia\\Lib\\YahnisElsts\\PluginUpdateChecker\\v5p7\\Vcs\\PluginUpdateChecker',
			'Vcs' . '\\ThemeUpdateChecker'  => 'Onumia\\Lib\\YahnisElsts\\PluginUpdateChecker\\v5p7\\Vcs\\ThemeUpdateChecker',
			'GitHubApi'                  => self::PUC_GITHUB_API_SUFFIX,
			'BitBucketApi'               => 'Onumia\\Lib\\YahnisElsts\\PluginUpdateChecker\\v5p7\\Vcs\\BitBucketApi',
			'GitLabApi'                  => 'Onumia\\Lib\\YahnisElsts\\PluginUpdateChecker\\v5p7\\Vcs\\GitLabApi',
		);
	...
	private function require_release_update_strategy( object $checker ): bool {
		...
		$checker->addFilter(
			...
			static function ( array $strategies ): array {
				return isset( $strategies[ self::PUC_STRATEGY_LATEST_RELEASE ] )
					? array( self::PUC_STRATEGY_LATEST_RELEASE => $strategies[ self::PUC_STRATEGY_LATEST_RELEASE ] )
					: array();
			},
			10,
			2
	...
	private function add_result_metadata( object $checker ): void {
		...
		$checker->addResultFilter(
			function ( mixed $info ): mixed {
				...
				$icons = array_filter(
					array(
						'svg'     => $this->config->icon_url( 'icon.svg' ),
						'default' => $this->config->icon_url( 'icon.png' ),
					),
					'is_string'
				);
