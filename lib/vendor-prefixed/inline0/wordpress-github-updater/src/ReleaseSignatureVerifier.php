...
final class ReleaseSignatureVerifier
{
    ...
    public function __construct(private readonly UpdaterConfig $config, private readonly GitHubReleaseUpdater $updater)
    {
    }
    ...
    private function candidate_for_package(string $package): ?array
    {
        ...
        $is_newer_candidate = 1 === preg_match(self::VERSION_PATTERN, $candidate_version) && version_compare($candidate_version, $this->config->current_version, '>');
        if ($candidate_package !== $package || !$is_newer_candidate) {
            return null;
        }
        return array('package' => $candidate_package, 'version' => $candidate_version, 'tag' => 'v' . $candidate_version);
    }
    private function candidate_field(mixed $candidate, string $field): string
    {
        $value = is_object($candidate) && isset($candidate->{$field}) ? $candidate->{$field} : (is_array($candidate) && isset($candidate[$field]) ? $candidate[$field] : '');
    ...
    private function fetch_release(array $repository, string $tag): ?array
    {
        ...
        $response = wp_remote_get($url, array('headers' => array_merge($this->auth_headers($repository), array('Accept' => 'application/vnd.github+json')), 'timeout' => self::REQUEST_TIMEOUT));
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            return null;
        }
        $decoded = json_decode((string) wp_remote_retrieve_body($response), \true);
    ...
    private function index_assets(array $release, array $repository): ?array
    {
        ...
        foreach ($release['assets'] as $asset) {
            ...
            $assets[$name] = array('download' => $api, 'urls' => array_values(array_filter(array($api, $browser))));
        }
        return $assets;
    }
    /**
    ...
    {
        $matches = array_filter($assets, static fn(array $asset): bool => in_array($candidate['package'], $asset['urls'], \true));
        if (1 !== count($matches)) {
            return null;
        }
    ...
    private function is_repository_asset_api_url(string $url, array $repository): bool
    {
        ...
        return is_array($parts) && 'https' === ($parts['scheme'] ?? '') && 'api.github.com' === ($parts['host'] ?? '') && 1 === preg_match('#^/repos/' . preg_quote($repository['owner'], '#') . '/' . preg_quote($repository['name'], '#') . '/releases/assets/[0-9]+$#', $path);
    }
    /**
     * @param array{owner:string,name:string} $repository Repository identity.
     */
    ...
    {
        $response = wp_remote_get($url, array('headers' => array_merge($this->auth_headers($repository), array('Accept' => 'application/octet-stream')), 'timeout' => self::REQUEST_TIMEOUT));
        if (is_wp_error($response) || 200 !== wp_remote_retrieve_response_code($response)) {
            return null;
        }
